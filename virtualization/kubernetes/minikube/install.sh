...
sudo ${MINIKUBE} start  --feature-gates=KubeletPodResources=true --image-mirror-country cn --image-repository=registry.cn-hangzhou.aliyuncs.com/google_containers --registry-mirror=https://49ri251n.mirror.aliyuncs.com --kubernetes-version=$version  --vm-driver=none --network-plugin=cni #--extra-config="kubeadm.pod-network-cidr=10.224.0.0/16"
